...
class InavService extends WebBaseService
{
     ...
     * @throws Exception
     */
    public function sendInvite($join_user, $inviteRoomRole)
    {
        if (array_key_exists($join_user->account_id, $this->getInviteList($join_user->room_id))) {
            $this->fail(ResponseCode::BUSINESS_REPEAT_INVITATION_THIRTY_LATER);
        }
        ...
        if ($speaker_count >= CachePrefixConstant::SPEAKER_MAX_NUM) {
            ...
            $this->sendMsg($join_user->room_id, [
                ...
                'invite_room_role' => $inviteRoomRole,//邀请人角色 主持人或助理
            ]);
        } else {
    ...
    {
        vss_logger()->info('csces-addSpeaker', ['action'=>'start', 'result' => ['time'=>date('Y-m-d H:i:s')]]);//日志
        if ($join_user->role_name != RoomJoinRoleNameConstant::HOST) {
            ...
            if (vss_redis()->get($key) != 2 &&
                ...
                vss_logger()->info('csces-addSpeaker', ['action'=>'addSpeaker', 'result' => ['key'=>$key, 'result'=>vss_redis()->get($key), 'time'=>date('Y-m-d H:i:s')]]);//日志
                vss_logger()->info('csces-addSpeaker', ['action'=>'getInviteList', 'result' => ['result'=>$this->getInviteList($join_user->room_id), 'time'=>date('Y-m-d H:i:s')]]);//日志
                $this->fail(ResponseCode::COMP_ROOM_ADDSPEAKER_TIMEOUT);
        ...
        if (!vss_redis()->hexists(
            ...
            if ($speaker_count < CachePrefixConstant::SPEAKER_MAX_NUM) {
                ...
                $delRes = vss_redis()->del([CachePrefixConstant::INTERACT_HANDSUP . $join_user->account_id]);
                vss_logger()->info('csces-addSpeaker', ['action'=>'delKey', 'result' => ['key'=>CachePrefixConstant::INTERACT_HANDSUP . $join_user->account_id, 'result'=>$delRes, 'time'=>date('Y-m-d H:i:s')]]);//日志
                $this->sendMsg($join_user->room_id, [
...
//        vss_service()->getSaasService()->syncSpeakSwitch($receive_join_user, 0);
    ...
    public function apply($join_user, $type)
    {
        ...
        if ($type == 1) {
            ...
            $setRes = vss_redis()->set($key, 1, CachePrefixConstant::HANDSUP_VALID_TIME);
            vss_logger()->info('csces-addSpeaker', ['action'=>'setKey', 'result' => ['key'=>$key, 'result'=>$setRes, 'time'=>date('Y-m-d H:i:s')]]);//日志
        } else {
            if (vss_redis()->get($key) != 1) {
                return true;
            }
     ...
     * @date    2020-08-13
     */
    public function get($ilId, $accountInfo, $password = '', $rolename = null, $from = 'js')
    {
        //房间信息
        ...
        if ($accountInfo) {
            ...
            $params    = [
                ...
                'avatar'        => $accountInfo['profile_photo'] ?: vss_config('application.static.headPortrait.default'),
                'role_name'     => $role,
                'device_type'   => $deviceType ?? 0,
                'device_status' => 0,
            ];
            $invitedRole = vss_service()->getRoomFormatService()->getRoleName($liveInfo['il_id'], $accountInfo['account_id'], $role);
            if($invitedRole){
                $params['role_name'] = $invitedRole;
    ...
    public function getRoomJoinsRoleName($liveInfo, $accountInfo, $password, $rolename)
    {
        ...
        if ($password) {
            if ($liveInfo['status'] == RoomConstant::STATUS_STOP) {
                return AccountConstant::ACCOUNT_TYPE_WATCH;
            }
