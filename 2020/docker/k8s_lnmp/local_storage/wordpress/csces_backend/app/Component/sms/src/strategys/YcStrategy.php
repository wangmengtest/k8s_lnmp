 ...
 *+----------------------------------------------------------------------
 */
class YcStrategy extends Strategy
{
    /**
     * 接口地址
     *
     * @var string
     */
    //public $baseUri = 'http://10.9.246.1:9310';
    public $baseUri = 'http://yc.cscecsteel.com';
    ...
    public function send(string $mobile, string $content):bool
    {
        switch ($this->contentSame) {
            case true:
                ...
                $response = $this->curlPost($this->baseUri . '/service/rest/sendSmsMsg', $data);
                /*$response = $this->post('/service/rest/sendSmsMsg', [
                    'pszSubPort' => '*&',
                    'userId'     => $this->userId,
     ...
     * @return bool
     */
    public function sendInviteSms(string $mobile, string $msg):bool
    {
    ...
    {
        return $this->send($mobile, '您本次验证码为' . $verifyCode . ', 本验证码3分钟内有效, 感谢您使用');
    }

    public function sendVerifyCodeBak(string $mobile, string $verifyCode):bool
    {
