...
use App\Component\room\src\constants\InavGlobalConstant;
...
class RoomService extends WebBaseService
{
    ...
    public function getByConsole($params)
    {
        ...
        if($roomInfo['notice_time'] == '0000-00-00 00:00:00' || empty($roomInfo['notice_time']) || (strpos($roomInfo['notice_time'], 'null') !== false)){
            $roomInfo['notice_time'] = '';
        }
        return $roomInfo;
    }
     ...
     * @throws \Exception
     */
    public function getListByFilter($condition, $page, $pageSize, $orderKey = ''){
        return $this->roomList($condition, $page, $pageSize, $orderKey);
    }

    //获取数量
    public function getListCountByFilter($condition){
        $model = vss_model()->getRoomsModel();
        $fields = ['rooms.il_id'];
        return $model->getListByFilter($condition, $fields);
    }
    ...
    public function roomList($condition, $page = 1, $pageSize = 10, $keyName = '')
    {
        ...
        $fields = ['*'];
        if($condition['source'] == 'watch-list' || $condition['source'] == 'invited-list'){
            $fields = ['rooms.*','room_invited.room_role'];
        }
        if($condition['source'] == 'owner-list' || $condition['source'] == 'manage-list'){
            $fields = ['rooms.*'];
        }
        ...
        if (!empty($liveList)) {
            ...
            foreach ($liveList as $key => $value) {
                ...
                $liveList[$key]['live_time'] = $time > 0 ? ceil($time / 60) : 0;
            }
        }
    ...
    public function setStatus($roomId, $status)
    {
        ...
        $this->setStartIlIds($room['il_id'], $status);
     ...
     * 记录正在开播的房间IDS(il_id)
     * */
    private function setStartIlIds($ilId, $status){
        $cacheKey = RoomConstant::ROOMS_START_IDS_CACHE . date('Y-m-d');
        if($status == RoomConstant::STATUS_START){
            vss_redis()->sadd($cacheKey, intval($ilId));
            vss_redis()->expire($cacheKey, 86400);
        }
        if($status == RoomConstant::STATUS_STOP){
            vss_redis()->srem($cacheKey, intval($ilId));
        }
    }
    ...
    public function join(array $params)
    {
        ...
        if (!$roomJoinModel) {
            ...
            vss_model()->getRoomJoinsModel()->updateRow($roomJoinModel->join_id, $data);
        }
    ...
    public function getOnlineList($roomId, $page, $pageSize, $condition = [])
    {
        ...
        if ($roomModel) {
            if (empty($condition['nickname']) && $page == 1) {
                ...
                vss_logger()->info('csces-getOnlineList', ['action'=>'getSpeakUsers-start', data=>['room_id'=>$roomId], 'result' => $speakList]);//日志
                //获取在线特殊角色用户
                $speakArr    = array_column($speakList, 'account_id');
                $specialList = $this->getSpecialUsers($roomId, $speakArr);
                vss_logger()->info('csces-getOnlineList', ['action'=>'getSpecialUsers-start', data=>['room_id'=>$roomId], 'result' => $specialList]);//日志

                //$specialList = array_merge($specialList, $speakList);
                $specialList = array_merge($speakList, $specialList);
                vss_logger()->info('csces-getOnlineList', ['action'=>'getSpecialUsers-end', data=>['room_id'=>$roomId], 'result' => $specialList]);//日志
                //判断是否在线
                $online = vss_service()->getPaasChannelService()->checkUserOnlineByChannel(
                    $roomModel->channel_id,
                    array_column($specialList, 'account_id')
                );
                vss_logger()->info('csces-getOnlineList', ['action'=>'checkUserOnlineByChannel-end', data=>['room_id'=>$roomId], 'result' => $online]);//日志
            ...
            if ($userIdList && is_array($userIdList)) {
                ...
                if (!empty($condition['nickname'])) {
                    $list = $list->where('nickname', trim($condition['nickname']));
                }
        ...
        //角色顺序：主讲人、主持人、嘉宾、助理、观众。
        $isMain = vss_service()->getInavService()->getMainScreen($roomId);
        vss_logger()->info('csces-get-online-list', ['action'=>'getDesktop-start', data=>['room_id'=>$roomId], 'result' => $isMain]); //日志
        $mainArr = $hostArr = [];
        foreach ($specialList as $key=>$user){
            if($user['account_id'] == $isMain && $user['role_name'] != RoomJoinRoleNameConstant::HOST){
                $mainArr[] = $user;
                unset($specialList[$key]);
            }
            if($user['role_name'] == RoomJoinRoleNameConstant::HOST){
                $hostArr[] = $user;
                unset($specialList[$key]);
            }
        }
        vss_logger()->info('csces-get-online-list', ['action'=>'getDesktop-end', data=>['room_id'=>$roomId], 'result' => $mainArr]); //日志
        return array_merge($mainArr, $hostArr, $specialList, $list);
    }
    ...
    public function getOnlineCount($roomId)
    {
        ...
        if ($roomModel) {
            vss_logger()->info('csces-checkOnlineCount', ['action'=>'getOnlineCount', 'start_time' => date('Y-m-d H:i:s')]); //日志
            $count = (int)vss_service()->getPaasChannelService()->getOnlineNumByChannel($roomModel->channel_id);
            vss_logger()->info('csces-checkOnlineCount', ['action'=>'getOnlineCount', 'end_time' => date('Y-m-d H:i:s')]); //日志
        }
    ...
    public function getSpecialList($roomId)
    {
        ...
        //角色顺序：主讲人、主持人、嘉宾、助理、观众。
        $isMain = vss_service()->getInavService()->getMainScreen($roomId);
        vss_logger()->info('csces-getSpecialList', ['action'=>'getDesktop-start', data=>['room_id'=>$roomId], 'result' => $isMain]); //日志
        $mainArr = $hostArr = [];
        foreach ($specialList as $key=>$user){
            if($user['account_id'] == $isMain && $user['role_name'] != RoomJoinRoleNameConstant::HOST){
                $mainArr[] = $user;
        ...
        vss_logger()->info('csces-getSpecialList', ['action'=>'getDesktop-end', data=>['room_id'=>$roomId], 'result' => ['main'=>$mainArr, 'host'=>$hostArr]]); //日志
        return array_merge($mainArr, $hostArr, $specialList);
        //return $specialList;
    }
    ...
    public function getSpecialUsers($roomId, $notAccountId = [])
    {
        ...
        $listSpecial = $listSpecial->orderByRaw("FIELD(role_name, " . implode(", ", [1, 5, 4, 3, 2]) . ")")->get()->toArray();
    ...
    public function syncStreamStatus($interactiveStreamStatus)
    {
        ...
        if ($interactiveStreamStatus['stream_status'] != 1) {
            ...
            if ($info) {
                ...
                //删除开播的ID
                $this->setStartIlIds($info['il_id'], RoomConstant::STATUS_STOP);
                vss_logger()->info('csces-CheckStreamStatusCrontab', ['action'=>'setStartIlIds', 'il_id' => $info['il_id'], 'time'=>date('Y-m-d H:i:s')]); //日志
                return $info->save();
            }
        }
        return false;
     ...
     * @return boolean
     */
    public function isWatchLive($roomId, $accountInfo){
        vss_logger()->info('csces-csces-checkOnlineCount', ['action'=>'isWatchLive', 'data' => $roomId, 'user_info' => $accountInfo]); //日志
        $roomInfo = vss_service()->getRoomService()->getRoomInfoByRoomId($roomId);
        vss_logger()->info('csces-csces-checkOnlineCount', ['action'=>'isWatchLive', 'data' => $roomInfo]); //日志
        if(empty($roomInfo)){
            $this->fail(ResponseCode::EMPTY_ROOM);
            return false;
        }
        ...
        $watchCount = vss_service()->getRoomListService()->watchCountList($watchCondition, $accountInfo);
        if($watchCount <= 0){
            $this->fail(ResponseCode::CHECK_IS_CAN_WATCH_LIVE);
        }
        return;

        //公开会议
        if($roomInfo['limit_type'] == WatchlimitConstant::ACCOUNT_TYPE_APPROVE){
            return true;
        }

        //自己创建的会议
        if($roomInfo['account_id'] == $accountInfo['account_id']){
            return true;
        }

        //邀请的人员
        $roomInvited = vss_service()->getRoomInvitedService()->getInvitesByIlId($roomInfo['il_id']);
        vss_logger()->info('csces-checkOnlineCount', ['action'=>'isWatchLive', 'data' => $roomInvited]); //日志
        if($roomInvited){
            if(in_array($accountInfo['account_id'], array_column($roomInvited, 'account_id'))){
                return true;
            }
        }

        if($this->checkCommonDepts($roomInfo, $accountInfo['org'])){
            return true;
        }
        $this->fail(ResponseCode::CHECK_IS_CAN_WATCH_LIVE);
    }
     ...
     * 检查是否同部门及以下的会议
     * */
    private function checkCommonDepts($roomInfo, $org){
        $orgId = vss_service()->getAccountOrgService()->orgIdByCode()[$org];
        if(empty($orgId)){
            $this->fail(ResponseCode::CHECK_IS_CAN_WATCH_LIVE);
        }
        $orgInfo = vss_model()->getAccountOrgModel()->getInfoByOrgId($orgId);
        if($orgInfo['orgs']){
            $orgs = (array)explode(',', $orgInfo['orgs']);
            if(in_array($roomInfo['org'], $orgs)){
                return true;
            }
        }
        if($orgInfo['depts']){
            $depts = (array)explode(',', $orgInfo['depts']);
            if(in_array($roomInfo['dept'], $depts)){
                return true;
            }
        }
        $this->fail(ResponseCode::CHECK_IS_CAN_WATCH_LIVE);
     ...
     * @return boolean
     */
    public function updateAccountNameByIlIdAndAccountId($ilId, $accountId, $nickname){
        $data = [];
        $data['il_id'] = $ilId;
        $data['account_id'] = $accountId;
        $data['account_name'] = $nickname;
        vss_service()->getRoomService()->update($data);
        vss_logger()->info('csces-room-account-sync', ['action'=>'syncAccountName-updateaccountname', 'data' => $data]); //日志
    }
     ...
     * @return boolean
     */
    public function updateAccountNameByAccountId($accountId, $nickname){
        $params = [];
        $params['account_id'] = $accountId;
        $params['pagesize'] = PHP_INT_MAX;
        $roomList = vss_service()->getRoomListService()->ownerList($params, ['account_id'=>$accountId]);

        if($roomList['data']){
            array_walk($roomList['data'], function ($item) use ($nickname, $accountId){
                $data = [];
                $data['il_id'] = $item['il_id'];
                $data['account_id'] = $accountId;
                $data['account_name'] = $nickname;
                vss_service()->getRoomService()->update($data);
            });
     ...
     * 检查嘉宾/助理口令
     * */
    public function checkRoomPassword($params){
        $ilId      = $params['il_id'];
        $password  = $params['password'];
        $rolename  = $params['role_name'];

        $supplyInfo = vss_model()->getRoomSupplyModel()->getInfoByIlId($ilId);
        if($rolename == AccountConstant::TYPE_ASSISTANT){
            if($password != $supplyInfo['assistant_sign']){
                $this->fail(ResponseCode::AUTH_ROLE_PASSWORD_ERROR);
            }
        }
        if($rolename == AccountConstant::TYPE_INTERACTION){
            if($password != $supplyInfo['interaction_sign']){
                $this->fail(ResponseCode::AUTH_ROLE_PASSWORD_ERROR);
            }
        }
    }
     ...
     * 恢复预告接口
     * */
    public function editStatusToWating($params){
        $roomInfo = vss_model()->getRoomsModel()->getInfoByIlId($params['il_id']);
        if (empty($roomInfo)) {
            $this->fail(ResponseCode::EMPTY_ROOM);
        }
        ...
        if($roomInfo['begin_time_stamp'] < time()){
            $this->fail(ResponseCode::CHECK_IS_CAN_STATUS_TO_WAITING);
        }
