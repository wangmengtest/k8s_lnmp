...
class StatService extends WebBaseService
{
    ...
    public function allData($accountId, $ilId, $beginTime, $endTime, $source)
    {
        ...
        if ($accountId) {
            ...
            $condition['watch_account_id_notin'] = [$accountId];
        }
        ...
        $uvStat          = vss_model()->getRoomAttendsAllModel()->getUvCount($condition);
        $data['live_uv'] = [
            'stat'   => $uvStat,
            'unit'   => '人',
        ...
        $data['max_concurrent'] = [
            'stat'   => ($maxUvStat > 0) ? ($maxUvStat - 1) : $maxUvStat,//减去主持人
            'unit'   => '人',
            'export' => '',
        ];
    ...
    public function liveData($accountId, $ilId, $beginTime, $endTime, $source)
    {
        ...
        // 直播时长
        unset($condition['watch_account_id_notin']);
    ...
    public function liveConcurrent($accountId, $ilId, $beginTime, $endTime)
    {
        ...
        foreach ($liveAttendList ?: [] as $key => $value) {
            $data[] = [
                ...
                'watch_count'         => ($value['watch_count'] > 0) ? ($value['watch_count'] - 1) : $value['watch_count'],
                'watch_account_count' => ($value['watch_count'] > 0) ? ($value['watch_count'] - 1) : $value['watch_count'],
            ];
        }
    ...
    public function exportUvData($export, $filePath)
    {
        ...
        $ilId   = $params['il_id'] ?? 0;
        vss_logger()->info('csces-exportuvdata', ['action'=>'start']); //日志
        ...
        vss_logger()->info('csces-exportuvdata', ['action'=>'start1']); //日志
        $exportProxyService = vss_service()->getExportProxyService()->init($file)->putRow($header);
        vss_logger()->info('csces-exportuvdata', ['action'=>'start2']); //日志
        $page     = 1;
        $pageSize = 1000;
        while (true) {
            vss_logger()->info('csces-exportuvdata', ['action'=>'start3', 'params'=>$params]); //日志
            $params['status'] = $params['type'] ?: 1;
            $params['begin_time'] = $params['start_time'] ?: '';
            unset($params['type']);
            $list = $this->getUvData($params, $page++, $pageSize);
            vss_logger()->info('csces-exportuvdata', ['condition'=>$params, 'result' => $list]); //日志
            if (empty($list)) {
                break;
            }
            $exportData = [];
            foreach ($list as $item) {
                $exportData[] = [
                    $item['nickname'],
                    $item['user_type'] == AccountConstant::USER_TYPE_CSCES ? '内部' : '外部',
                    $item['org_name'] ?? '',
                    $item['dept_name'] ?? '',
                    $item['duration'] ? $item['duration'] . '分钟' : ''
                ];
            }
            vss_logger()->info('csces-exportuvdata', ['action'=>'format-data', 'result' => $exportData]); //日志
     ...
     * @date 2021/2/26
     */
    public function exportUvDataBak($export, $filePath)
    {
        $params = json_decode($export['params'], true);
        $header = json_decode($export['title'], true);
        $file   = $filePath . $export['file_name'];
        ...
        $queryListFunc = function (Builder $model, $page, $pageSize) {
            return $model->groupBy(['watch_account_id'])
                ...
                ->selectRaw('id, il_id, watch_account_id, duration')
                ->get()
                ->toArray();
        };
        ...
        $exportDataFunc = function ($baseModel, $type = '直播') use ($queryListFunc, $exportProxyService, $roomInfo) {
            ...
            while (true) {
                ...
                $accountMap = vss_model()->getAccountsModel()
                    ...
                    ->get(['account_id', 'username', 'nickname', 'user_type', 'dept', 'org'])
                    ->toArray();
                ...
                foreach ($list as $item) {
                    ...
                    $exportData[] = [
                        ...
                        ($accountMap[$accountId]['user_type'] == 2) ? '内部' : '外部',
                        $orglist[$accountMap[$accountId]['org']] ?? '',
                        $orglist[$accountMap[$accountId]['dept']] ?? '',
                        $item['duration']
                    ];
                }
    ...
    public function exportPvData($export, $filePath)
    {
        ...
        $header    = json_decode($export['title'], true);
        $ext       = 'csv';
        $file      = $filePath . $export['file_name'] . '.' . $ext;
        $page      = 1;
        ...
        if ($params['account_id']) {
            ...
            $condition['watch_account_id_notin'] = [$params['account_id']];
        }
        $liveCondition    = $condition;
        $modelRoomAttends = vss_model()->getRoomAttendsModel();
    ...
    /*查询观看人数*/
    public function getUvListsByFilter($params, $accountInfo = []){
        $page      = $params['page'] ?: 1;
        $pageSize  = $params['page_size'] ?: 6;

        return [
            'current_page' => $page,
            'data' => $this->getUvData($params, $page, $pageSize),
            'per_page' => $pageSize,
            'total' => $this->getUvCount($params, $accountInfo)
        ];
    }

    /*获取总数*/
    public function getUvCount($params, $accountInfo = []){
        $ilId      = $params['il_id'];
        $beginTime = $params['begin_time'];
        ...
        $condition['attend_type_in'] = $attendType;

        //3.1、直播累计观众人数
        return vss_model()->getRoomAttendsAllModel()->getUvCount($condition);
    }

    /*获取列表数据*/
    public function getUvData($params, $page = 1, $pageSize = 6){
        $ilId      = $params['il_id'];
        $beginTime = $params['begin_time'];
        $endTime   = $params['end_time'] . ' 23:59:59';
        $accountId = $params['account_id'];
        ...
        $buildModelFunc = function (Model $model) use ($ilId, $accountId, $beginTime, $endTime, $attendType) {
            return $model->where('account_id', $accountId)
                ->whereBetween('start_time', [$beginTime, $endTime])
                ->where('watch_account_id', '>', 0)
                ->where('watch_account_id', '<>', $accountId)
                ->whereIn('attend_type', $attendType)
                ->when($ilId, function ($query) use ($ilId) {
                    $query->where('il_id', $ilId);
                });
        };
        ...
        $queryListFunc = function (Builder $model, $page, $pageSize) {
            return $model->groupBy(['watch_account_id'])
                ->orderByDesc('end_time')
                ->forPage($page, $pageSize)
                ->selectRaw('id, il_id, watch_account_id, sum(duration) as duration')
                ->get()
                ->toArray();
        };
        $baseModel = $buildModelFunc(vss_model()->getRoomAttendsAllModel());
        $list = $queryListFunc(clone $baseModel, $page, $pageSize);
        // 查询最早开始时间
        $watchAccountIds = array_column($list, 'watch_account_id');
        // 查询用户昵称和账号
        $accountMap = vss_model()->getAccountsModel()
            ->whereIn('account_id', $watchAccountIds)
            ->get(['account_id', 'username', 'nickname', 'org', 'dept', 'user_type'])
            ->toArray();
        $accountMap = array_column($accountMap, null, 'account_id');
        $orglist = vss_service()->getAccountOrgService()->orgDataByCode();
        array_walk($list, function (&$item) use($orglist, $accountMap) {
            $item['duration'] = $item['duration'] > 0 ? ceil($item['duration'] / 60) : 0;
            $item['org_name'] = $orglist[$accountMap[$item['watch_account_id']]['org']] ?? '';
            $item['dept_name'] = $orglist[$accountMap[$item['watch_account_id']]['dept']] ?? '';
            $item['nickname'] = $accountMap[$item['watch_account_id']]['nickname'] ?? '';
            $item['user_type'] = $accountMap[$item['watch_account_id']]['user_type'] ?? '';
        });
        return $list;
    }
}
