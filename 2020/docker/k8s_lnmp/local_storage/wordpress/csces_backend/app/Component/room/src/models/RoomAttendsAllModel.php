...
use App\Component\room\src\constants\RoomAttendsAllConstant;
 ...
 * @property bool   $attend_type  数据来源 1-直播 2-回放
 ...
 */
class RoomAttendsAllModel extends WebBaseModel
{
    /**
    ...
    protected $table = 'room_attends_all';
    ...
    protected function buildCondition(Builder $model, array $condition): Builder
    {
        ...
        //观看类型
        $model->when($condition['attend_type_in'] ?? '', function ($query) use ($condition) {
            $query->whereIn('attend_type', $condition['attend_type_in']);
        });
    ...
    public function createRecordAttends(int $ilId, int $accountId, string $recordId, array $data)
    {
        ...
        $this->attend_type  = RoomAttendsAllConstant::ATTEND_TYPE_RECORD;
    ...
    public function createLiveAttends($ilId, $accountId, $data, $type)
    {
        ...
        $this->province     = $data['viewer_province'];
        $this->type         = $type;
        $this->attend_type  = RoomAttendsAllConstant::ATTEND_TYPE_ROOM;
        $this->created_time = $data['created_time'];
        $this->updated_at   = $datetime;
        $this->created_at   = $datetime;
        $insert             = $this->save();
     ...
     * @author fym
     */
    public function syncRecordData(array $item, int $ilId, int $accountId)
    {
        $condition = [
        ...
        // 查询是否存在
        $recordAttendsId = $this->where($condition)->value('id');

        // 存在则修改，不存在则新增
        if ($recordAttendsId) {
            $datetime = date('Y-m-d H:i:s');
            return $this->where('id', $recordAttendsId)
                ->update([
        ...
        return $this->createRecordAttends(
            $ilId,
            $accountId,
            $item['record_id'],
            $item
    ...
    public function syncData($roomInfo, $item, $type)
    {
        ...
        $condition    = [
            ...
            'country'          => $item['viewer_country'],
            'province'         => $item['viewer_province'],
            'attend_type'      => RoomAttendsAllConstant::ATTEND_TYPE_ROOM,
        ];
