 ...
 * @property string  $user_type  用户类型 2:中建内部用户
...
class AccountsModel extends WebBaseModel
{
     ...
     * @var array
     */
    protected $fillable = ['phone', 'username', 'nickname', 'sex', 'token', 'status', 'type', 'account_type', 'third_user_id', 'org', 'org_name', 'user_type', 'dept', 'role_id', 'user_id', 'password', 'pro_id', 'c_user_id', 'deleted_at'];
    ...
    public static function boot()
    {
        ...
        static::saving(function ($model) {
            if ($model->user_type == AccountConstant::USER_TYPE_CSCES) {
                $model->deleteCache('CscesAllUser', 'all');
            }
            // 昵称长度禁止超过20个字符
        ...
        self::updated(function (self $data) {
            ...
            if ($data->user_type == AccountConstant::USER_TYPE_CSCES) {
                $data->deleteCache('CscesAllUser', 'all');
            }
        });
    ...
    public static function getTypeStr($type): string
    {
        switch ($type) {
            ...
            case AccountConstant::TYPE_ASSISTANT:
                $string = '助理';
                break;
            case AccountConstant::TYPE_FLYING:
                $string = '飞手';
                break;
            default:
                $string = '未知';
    ...
    public function updateTokenByAccountId($accountId, $token)
    {
        $condition = [
     ...
     * @return AccountsModel|null
     */
    public function getCscesAllUser()
    {
        $attributes = $this->getCache('CscesAllUser', 'all', function (){
            //return $this->whereIn('user_type', [AccountConstant::USER_TYPE_CSCES])->get()->toArray();
            return $this->where('status', AccountConstant::STATUS_ENABLED)->whereIn('user_type', [AccountConstant::USER_TYPE_CSCES])->select('account_id','user_id','username','nickname','org','dept')->get()->toArray();
        });
        return $attributes;
    }
    ...
    protected function buildCondition(Builder $model, array $condition) :Builder
    {
        ...
        $model->when($condition['org_ids'], function ($query) use ($condition) {
            $query->whereIn('accounts.org', $condition['org_ids']);
        });

        $model->when($condition['dept_ids'], function ($query) use ($condition) {
            $query->whereIn('accounts.dept', $condition['dept_ids']);
        });
